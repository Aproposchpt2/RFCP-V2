'use strict';
// subscriber-capability.js — Capability Report for active subscribers
// GET with Authorization: Bearer <session_token>
// Verifies session → loads demo_snapshots profile → fetches live SAM.gov entity → returns capability HTML
...
function buildHtml(r) {
  ...
  return '<!DOCTYPE html><html><head><meta charset="utf-8"><style>'
    ...
    + '<div class="section"><h2>Core Competencies</h2><p class="no-data">Log in to your CapGen profile to add core competencies and differentiators to this report.</p></div>'
    ...
    + '</div></div>'
    + '<div class="section"><h2>NAICS Codes</h2><div class="data naics">' + (naics || '<p class="no-data">No NAICS codes on record.</p>') + '</div></div>'
    + '</div></div>'
    + '<div class="footer"><div style="color:#0A1A3A"><b>' + esc(r.legal_name) + '</b></div>'
    + '<div style="text-align:right;color:#43506a;line-height:1.5">UEI ' + esc(r.uei) + ' &middot; CAGE ' + esc(r.cage) + '</div></div>'
    + '<div class="engine">Generated by CapGen &middot; Sourced from official public records (SAM.gov).</div>'
    + '</div></body></html>';
}
...
exports.handler = async function(event) {
  ...
  // Auth
  var email = await verifySession(event.headers.authorization || event.headers.Authorization || '');
  ...
  var snapRes = await fetch(
    SUPABASE_URL + '/rest/v1/demo_snapshots?requester_email=eq.' + encodeURIComponent(email) + '&order=created_at.desc&limit=1',
    { headers: sbH() }
  );
  if (!snapRes.ok) return { statusCode: 502, headers: CORS, body: JSON.stringify({ error: 'DB error' }) };
  var snaps = await snapRes.json();
  if (!snaps.length) return { statusCode: 404, headers: CORS, body: JSON.stringify({ error: 'NO_PROFILE' }) };

  var snap    = snaps[0];
  var profile = snap.profile || {};
  var uei     = profile.uei || snap.entity_uei || '';

  if (!uei) return { statusCode: 404, headers: CORS, body: JSON.stringify({ error: 'NO_UEI' }) };
  if (!SAM_API_KEY) return { statusCode: 500, headers: CORS, body: JSON.stringify({ error: 'SAM_API_KEY not configured' }) };
  ...
  return {
    ...
    body: JSON.stringify({ html: html, legal_name: entity.legal_name }),
  };
};
